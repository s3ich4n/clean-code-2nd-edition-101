...
Java에서는 Spring 프레임워크가 DI 컨테이너를 제공하는 것으로 유명하다. XML config 파일을 통해서 어느 객체가 서로 연결될 지를 결정하고, Java 코드에서 이름으로 특정 객체를 요청한다. 대다수의 DI 컨테이너는 팩토리를 호출하거나 프록시를 생성하는 메커니즘을 가진다.
